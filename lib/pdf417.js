...
var PDF417 = {
	...
	*/
	draw: function(code, canvas, aspectratio, ecl, devicePixelRatio, lineColor) {
		...
		if (numcw > 925) {
			// PDF417 spec limit: max 925 data codewords (~1850 text chars or ~1110 bytes)
			throw new Error('Input too large for PDF417: encoded to ' + numcw + ' codewords, maximum is 925.');
		}
		...
		if (pad > 0) {
			if ((size - rows) == nce) {
				...
				// add pading
				codewords = codewords.concat(new Array(pad).fill(900));
			}
		}
		...
		const pstart = '0'.repeat(this.QUIETH) + this.start_pattern;
		const pstop = this.stop_pattern + '0'.repeat(this.QUIETH);
		barcode_array.num_rows = (rows * this.ROWHEIGHT) + (2 * this.QUIETV);
		barcode_array.num_cols = ((cols + 2) * 17) + 35 + (2 * this.QUIETH);
		barcode_array.bcode = [];

		let empty_row;
		// build rows for vertical quiet zone
		if (this.QUIETV > 0) {
			empty_row = '0'.repeat(barcode_array.num_cols);
			for (let i = 0; i < this.QUIETV; ++i) {
		...
		for (let r = 0; r < rows; ++r) {
			...
			let row = pstart;
			switch (cid) {
				case 0: L = ((30 * Math.trunc(r / 3)) + Math.trunc((rows - 1) / 3)); break;
				case 1: L = ((30 * Math.trunc(r / 3)) + (ecl * 3) + ((rows - 1) % 3)); break;
				case 2: L = ((30 * Math.trunc(r / 3)) + (cols - 1)); break;
			}
			// left row indicator
			row += this.clusters[cid][L].toString(2).padStart(17, '0');
			// for each column
			for (let c = 0; c < cols; ++c) {
				row += this.clusters[cid][codewords[k]].toString(2).padStart(17, '0');
				++k;
			}
			switch (cid) {
				case 0: L = ((30 * Math.trunc(r / 3)) + (cols - 1)); break;
				case 1: L = ((30 * Math.trunc(r / 3)) + Math.trunc((rows - 1) / 3)); break;
				case 2: L = ((30 * Math.trunc(r / 3)) + (ecl * 3) + ((rows - 1) % 3)); break;
			}
			// right row indicator
			row += this.clusters[cid][L].toString(2).padStart(17, '0');
			// row stop code
			...
			for (let h = 0; h < this.ROWHEIGHT; ++h) {
				barcode_array.bcode.push(row);
			}
			++cid;
			if (cid > 2) {
				cid = 0;
		...
		const retinaMultiplier = devicePixelRatio || (typeof window !== 'undefined' ? window.devicePixelRatio || 1 : 1);
		canvas.width = barcode_array.num_cols * retinaMultiplier;
		canvas.height = barcode_array.num_rows * retinaMultiplier;
		const ctx = canvas.getContext('2d');
		ctx.scale(retinaMultiplier, retinaMultiplier);
		ctx.fillStyle = lineColor;

		// graph barcode elements; batch consecutive black pixels into a single fillRect per row
		const numRows = barcode_array.num_rows;
		const numCols = barcode_array.num_cols;
		for (let r = 0; r < numRows; ++r) {
			const rowStr = barcode_array.bcode[r];
			let c = 0;
			while (c < numCols) {
				if (rowStr.charCodeAt(c) === 49 /* '1' */) {
					let start = c;
					while (c < numCols && rowStr.charCodeAt(c) === 49) ++c;
					ctx.fillRect(start, r, c - start, 1);
				} else {
					++c;
				}
			}
	...
	getInputSequences: function(code) {
		...
		for(var i=0; i<numseq.length; i++) {
			...
			if (seq[1] > 0) {
				...
				var prevseq = code.substring(offset, seq[1]);
				var textseq = [];
				// get text sequences
				...
				for(var j=0; j<textseq.length; j++) {
					...
					if (txtseq[1] > 0) {
						...
						var prevtxtseq = prevseq.substring(txtoffset, txtseq[1]);
		...
		switch(mode) {
			case 900: { // Text Compaction mode latch
				...
				var codelen = code.length;
					...
					var chval = code.charCodeAt(i);
					var k;
					if ((k = this.textsubmodes[submode].indexOf(chval)) !== -1) {
						...
						for (var s = 0; s < 4; ++s) {
							// search new sub-mode
							if ((s != submode) && ((k = this.textsubmodes[s].indexOf(chval)) !== -1)) {
								// s is the new submode
								if ((((i + 1) == codelen) || (((i + 1) < codelen) && (this.textsubmodes[submode].indexOf(code.charCodeAt(i + 1)) !== -1))) && ((s == 3) || ((s == 0) && (submode == 1)))) {
			...
			case 924: { // Byte Compaction mode latch
				...
				while ((codelen = code.length) > 0) {
					...
					if (sublen == 6) {
						var t = BigInt(code.charCodeAt(0)) * 1099511627776n
						      + BigInt(code.charCodeAt(1)) * 4294967296n
						      + BigInt(code.charCodeAt(2)) * 16777216n
						      + BigInt(code.charCodeAt(3)) * 65536n
						      + BigInt(code.charCodeAt(4)) * 256n
						      + BigInt(code.charCodeAt(5));
	...
	getErrorCorrection: function(cw, ecl) {
		...
		var ecw = new Array(eclsize).fill(0);
